...
# Notes:
# - Set SEED_LIMIT <- NA to run on the full dataset (slow, many API calls).
# - Caching is used to avoid re-hitting Wikidata repeatedly.
...
## Load input (rich list)
# Input: Excel with columns including id_wikidata (Q-IDs), id_fam, name.
# We keep one row per Wikidata ID and count distinct families for context.
...
# how many rows have a valid Wikidata entity (Q-IDs)? 439, filters duplicates
...
##### 2.Crawl parents (upward) #####
# We build ancestor trees by following P22 (father) and P25 (mother).
# The crawler uses polite rate limits and a small retry/backoff loop.
# Results are cached per Q-ID under cache/parents/.
...
# After collecting parents, we also pull "child -> parent" edges
# by querying all known parents for their children. This fills in
# co-parent pairs and makes sibling inference far more complete.
# Cached under cache/children/.
...
# Query all children of a parent (both sides) 
...
#Augment your existing edges/nodes with children
augment_graph_with_children <- function(edges, nodes) {
  parent_set <- unique(edges$to)
...
#### 2b. Direct horizontal fetchers (Wikidata)(P26 spouse, P3373 sibling) ####
# Some spouse/sibling links are recorded directly in Wikidata:
# - P26  = spouse
# - P3373 = sibling
# We fetch those too so we don't miss couples/siblings without
# visible parent/child links. Cached per Q-ID.
...
# Build undirected spouse edges from P26 for a vector of QIDs
...
# Build undirected sibling edges from P3373 for a vector of QIDs
...
# BFS from each seed upward through P22/P25 to collect ancestors.
# We keep (from, to, type) edges and a minimal label map for nodes.
